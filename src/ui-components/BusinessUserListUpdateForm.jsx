...
export default function BusinessUserListUpdateForm(props) {
  const {
    id: idProp,
    businessUserList: businessUserListModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = businessUserListRecord
      ? { ...initialValues, ...businessUserListRecord }
      : initialValues;
    setName(cleanValues.Name);
    setEmail(cleanValues.Email);
    setContactPersonName(cleanValues.ContactPersonName);
    setContactPersonEmail(cleanValues.ContactPersonEmail);
    setCampaignName(cleanValues.CampaignName);
    setTrackingNumber(cleanValues.TrackingNumber);
    setMonthlyFee(cleanValues.MonthlyFee);
    setCostPerSMS(cleanValues.CostPerSMS);
    setCallCostPerMinute(cleanValues.CallCostPerMinute);
    setErrors({});
  };
  const [businessUserListRecord, setBusinessUserListRecord] = React.useState(
    businessUserListModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(BusinessUserList, idProp)
        : businessUserListModelProp;
      setBusinessUserListRecord(record);
    };
    queryData();
  }, [idProp, businessUserListModelProp]);
  React.useEffect(resetStateValues, [businessUserListRecord]);
  const validations = {
    Name: [],
    Email: [{ type: "Email" }],
    ContactPersonName: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            BusinessUserList.copyOf(businessUserListRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "BusinessUserListUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || businessUserListModelProp)}
