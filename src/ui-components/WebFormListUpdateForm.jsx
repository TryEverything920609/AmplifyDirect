...
export default function WebFormListUpdateForm(props) {
  const {
    id: idProp,
    webFormList: webFormListModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = webFormListRecord
      ? { ...initialValues, ...webFormListRecord }
      : initialValues;
    setPrivilege(cleanValues.Privilege);
    setEmail(cleanValues.Email);
    setUser(cleanValues.User);
    setLogo(cleanValues.Logo);
    setErrors({});
  };
  const [webFormListRecord, setWebFormListRecord] =
    React.useState(webFormListModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(WebFormList, idProp)
        : webFormListModelProp;
      setWebFormListRecord(record);
    };
    queryData();
  }, [idProp, webFormListModelProp]);
  React.useEffect(resetStateValues, [webFormListRecord]);
  const validations = {
    Privilege: [],
    Email: [{ type: "Email" }],
    User: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            WebFormList.copyOf(webFormListRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "WebFormListUpdateForm")}
      {...rest}
    >
      <TextField
        label="Privilege"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || webFormListModelProp)}
