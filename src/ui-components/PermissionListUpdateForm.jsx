...
export default function PermissionListUpdateForm(props) {
  const {
    id: idProp,
    permissionList: permissionListModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = permissionListRecord
      ? { ...initialValues, ...permissionListRecord }
      : initialValues;
  ...
  const [permissionListRecord, setPermissionListRecord] = React.useState(
    permissionListModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(PermissionList, idProp)
        : permissionListModelProp;
      setPermissionListRecord(record);
    };
    queryData();
  }, [idProp, permissionListModelProp]);
  React.useEffect(resetStateValues, [permissionListRecord]);
  const validations = {
    icon: [],
    label: [],
    to: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            PermissionList.copyOf(permissionListRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "PermissionListUpdateForm")}
      {...rest}
    >
      <TextField
        label="Icon"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || permissionListModelProp)}
