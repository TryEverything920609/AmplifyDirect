...
export default function CallLogListUpdateForm(props) {
  const {
    id: idProp,
    callLogList: callLogListModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = callLogListRecord
      ? { ...initialValues, ...callLogListRecord }
      : initialValues;
  ...
  const [callLogListRecord, setCallLogListRecord] =
    React.useState(callLogListModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(CallLogList, idProp)
        : callLogListModelProp;
      setCallLogListRecord(record);
    };
    queryData();
  }, [idProp, callLogListModelProp]);
  React.useEffect(resetStateValues, [callLogListRecord]);
  const validations = {
    From: [{ type: "Phone" }],
    To: [{ type: "Phone" }],
    Cost: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            CallLogList.copyOf(callLogListRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "CallLogListUpdateForm")}
      {...rest}
    >
      <TextField
        label="From"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || callLogListModelProp)}
